...
\[
\text{EI}_z
= 0.5 \cdot \text{volatility}_z
+ 0.3 \cdot \text{jump\_frequency}_z
+ 0.2 \cdot \text{turnover}_z
\]
...
\(\text{EI}_z > 0\) are labeled **extroverted**, indicating above-average behavioral intensity, while assets with  
\(\text{EI}_z \le 0\) are labeled **introverted**, corresponding to below-average market activity. No additional thresholds or nonlinear transformations are introduced; the classification boundary coincides with the cross-sectional mean.
